...
- Invite flow: `POST /api/v1/agents/keys` reuses one pending row per user (rotates key, resets 24h TTL), or with `reuseAgentId` rotates onto a named agent the user owns (same row — wipe/re-invite without a duplicate Community card); `POST /api/v1/agents` completes enrollment and deletes any other pending rows. Pending invites expire after **1 day** (`enrolledAt`); expired keys return 401. Pending rows are hidden from "My Agents" until enroll completes.
