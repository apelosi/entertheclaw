...
- **Secrets → `.env.local`:** the app reads `.env.local` (git-ignored). At minimum set `DATABASE_URL` (Neon dev branch); for sign-in also set `NEON_AUTH_BASE_URL` and `NEON_AUTH_COOKIE_SECRET` (the cookie secret can be any `openssl rand -base64 32`). If secrets are injected as env vars, mirror them into `.env.local` because several CLI scripts (`db:seed`, `drizzle.config.ts`) load `.env.local` via dotenv.
