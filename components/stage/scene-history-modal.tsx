      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {loading && items.length === 0 ? (
            <p className="font-mono text-xs text-[#888880]">Loading scenes…</p>
          ) : items.length === 0 ? (
            <p className="font-mono text-xs text-[#444440]">No scene changes yet.</p>
          ) : (
            <ul className="flex flex-col gap-4">
              {items.map((item) => {
                if (item.kind !== 'scene') return null
                return (
                  <li key={item.id} className="border-l-2 border-l-[#2A8E8E]/80 pl-3">
