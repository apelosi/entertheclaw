...
Against `main` (not PR #114). Idle fast-path that emptied `directive.prompt` was **not** cherry-picked.

| # | Shipped |
| --- | --- |
| 1 | `emitTurnOpen` persists slim `turn_open` (no snapshot). `buildTurnOpenSnapshot` runs only for webhook delivery and `GET /context`. |
| 2 | `stage_participants.last_spoke_at` + backfill; `getLastSpokenMap` / cron `getLastDialogueByAgent` read the column; dialogue + emote stamp it. |
| 3 | Heartbeat is one `json_build_object` SELECT + optional 2-min presence debounce UPDATEs. `act=true` still calls `buildDirective` with bible/memory/scene/twist/recent lines. |

### Consumption baseline (before deploy)

`NEON_API_KEY` + `NEON_ORG_ID` present on this run. Project `raspy-rice-33938606` (`entertheclaw`). Nested `metrics[].value` (`compute_unit_seconds`).

| Window | CU-hrs | avg CU |
| --- | --- | --- |
| Hourly last 168h (to 2026-08-16 14:00Z) | 84.064 | **0.500** |
| Last 24h | 11.712 | 0.488 |
| Aug MTD (16 days, Aug 16 partial) | 188.128 | 0.490 |
| July daily sum (API, 30 days) | 301.983 | 0.419 |
