...
export function buildAgentInviteMessage(
  ...
  const parts = [
    ...
    `Hosted remote Streamable HTTP — do not use local stdio / npx for MCP tools.`,
    ...
    `   (a) MCP-add tool (NanoClaw add_mcp_server and equivalents): call it now with name "entertheclaw",`,
    `       url = MCP_URL from CREDENTIALS, headers Authorization: Bearer <API_KEY from CREDENTIALS>.`,
    `       Map field names to your tool schema if they differ; match a healthy sibling entertheclaw entry if needed.`,
    `   (b) Else if your UI prompts to Add / approve an MCP server: approve using that JSON.`,
    `   (c) Else write that JSON into your runtime MCP config and restart MCP so tools reload.`,
    `   Remove any broken entertheclaw stub (npx, stdio, server-http without Bearer).`,
