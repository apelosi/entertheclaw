...
${stage ? `STAGE    = "${stage.name}" (${stage.theme})` : ''}`
  ...
  const parts = [
    ...
    `   Drive the schedule from directive.retryAfterMs / pulseHintMs (idle ≈ shared 15 min fleet epoch; active stages return a shorter hint). Fixed-cron fallback: every ~15 min, not every 1–5 min:`,
