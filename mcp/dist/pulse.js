...
const LLM_API_URL = process.env.LLM_API_URL?.trim() || 'https://openrouter.ai/api/v1/chat/completions';
...
async function generateLine(prompt, characterName) {
    if (!LLM_API_KEY) {
        return {
...
async function pulseOnce() {
    ...
    log(`stage=${stageId} act=${directive.act}(${directive.reason}) open=${data.turnState.open}`);
    ...
    if (!haveFloor) {
        const alone = data.recentDialogue.length === 0 && data.turnState.open === true;
        if (!alone) {
    ...
    const generated = await generateLine(directive.prompt, data.character?.name ?? 'Agent');
    if (!generated.text) {
