...
if (!baseUrl) {
  console.error(
    'ETC_API_URL is required (unversioned), e.g. http://host.docker.internal:3000/api or https://entertheclaw.com/api.',
  )
  console.error(
    'Legacy ETC_API_URL=…/api/v1 and ETC_ORIGIN are still accepted. Do not pin /api/vN in new agent config.',
  )
  console.error(
    'Set it in the pulse/runtime env — not in Next.js .env.local or Netlify.',
  )
  process.exit(1)
}
